...
6. Set `MODEL_ENCRYPTION_KEY` to a base64-encoded 32-byte secret, configure `CLAMAV_PATH`, and provide Ed25519 `MODEL_SIGNING_PRIVATE_KEY`/`MODEL_SIGNING_PUBLIC_KEY`; production startup rejects missing values.
7. Run database migrations as a release job, not on every horizontally scaled web restart.
8. Run `npm start` for the API, `npm run start:worker` as one separately scaled indexer instance, and `npm run start:benchmark-worker` as a separate metadata-safe benchmark worker. Add durable progress, retries, and alerts before enabling inference execution.
...
8. Perform a wallet-switch, unauthorized-route, upload-rejection, signed-manifest, benchmark-isolation, purchase, and access-delivery smoke test.
