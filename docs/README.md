...
NEXT_PUBLIC_CONTRACT_ADDRESS_LICENSE_NFT=0x...
NEXT_PUBLIC_CONTRACT_ADDRESS_REVIEW_ANCHOR=0x...
...
For the conference-level blockchain path, deploy the Phase 9 bundle. It adds
artifact SHA-256 anchoring, transferable AI License NFTs, primary and resale
license events, automatic ERC-2981 royalties, and verified immutable review
anchors:

```powershell
cd contracts
npx hardhat run scripts/deploy-phase9.ts --network sepolia
```

Set `REGISTRY_ADDRESS` to a Phase 9-compatible `AIModelRegistry` when reusing
an existing deployment. The generated deployment file contains
`registry`, `marketplaceV2`, `licenseNFT`, `reviewAnchor`, and `accessManagerV2`.
Configure `LICENSE_NFT_ADDRESS` and `REVIEW_ANCHOR_ADDRESS` in the backend, and
the matching `NEXT_PUBLIC_CONTRACT_ADDRESS_LICENSE_NFT` and
`NEXT_PUBLIC_CONTRACT_ADDRESS_REVIEW_ANCHOR` values in the frontend. When the
governance signer is a multisig/timelock, grant `MINTER_ROLE` to the marketplace
and call `setLicenseNFT` through governance before opening listings.
...
`PORT`, `NODE_ENV`, `DATABASE_URL`, `JWT_SECRET`, `ACCESS_TOKEN_TTL_SECONDS`, `REFRESH_TOKEN_TTL_SECONDS`, `SESSION_IDLE_TIMEOUT_MINUTES`, `TRUST_PROXY`, `REQUEST_SIGNING_MAX_SKEW_SECONDS`, `API_KEYS`, `FRONTEND_URL`, `ADMIN_WALLET_ADDRESS`, `BACKEND_PUBLIC_URL`, `RPC_URL`, `CHAIN_ID`, `INDEXER_START_BLOCK`, `REGISTRY_ADDRESS`, `MARKETPLACE_ADDRESS`, `LICENSE_NFT_ADDRESS`, `REVIEW_ANCHOR_ADDRESS`, `ACCESS_MANAGER_ADDRESS`, `BACKEND_SIGNER_PRIVATE_KEY`, `PINATA_JWT`, `PINATA_GATEWAY_JWT`, `PINATA_GATEWAY`, `MODEL_ENCRYPTION_KEY`, `CLAMAV_PATH`, `MODEL_SIGNING_PRIVATE_KEY`, `MODEL_SIGNING_PUBLIC_KEY`, `MODEL_SECURITY_SCORE_THRESHOLD`, `BENCHMARK_RUNNER_PATH`, `EMBEDDING_MODEL`, `MAX_UPLOAD_BYTES`, `UPLOAD_TEMP_DIR`

### Frontend

`NEXT_PUBLIC_BACKEND_URL`, `NEXT_PUBLIC_RPC_URL`, `NEXT_PUBLIC_CHAIN_ID`, `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID`, `NEXT_PUBLIC_CONTRACT_ADDRESS_REGISTRY`, `NEXT_PUBLIC_CONTRACT_ADDRESS_MARKETPLACE`, `NEXT_PUBLIC_CONTRACT_ADDRESS_LICENSE_NFT`, `NEXT_PUBLIC_CONTRACT_ADDRESS_REVIEW_ANCHOR`, `NEXT_PUBLIC_CONTRACT_ADDRESS_ACCESS`
